...
describe('RareFiVault Contract Tests', () => {
  ...
  describe('Edge Cases', () => {
    ...
    it('should reject swapYield when slippage exceeds maxSlippageBps', async () => {
      const deployment = await deployVaultForTest(algod, creator, {
        minSwapThreshold: 2_000_000,
      });
      ...
      await expect(
        performSwapYield(algod, deployment, creator, 10_000_000, 5001)
      ).rejects.toThrow();
  ...
  describe('Farm Feature - Comprehensive Tests', () => {
    ...
    describe('Farm Edge Cases', () => {
      it('should handle swap when farm is empty but emission ratio set', async () => {
        const deployment = await deployVaultForTest(algod, creator, {
          creatorFeeRate: 0,
          minSwapThreshold: 2_000_000,
        });
      ...
      it('should apply creator fee on total output including farm bonus', async () => {
        ...
        // Setup farm: 500 IBUS, high emission ratio to get a big bonus
        await fundAsset(algod, creator, creator.addr, deployment.ibusAssetId, 1_000_000_000);
        await performContributeFarm(algod, deployment, creator, 500_000_000); // 500 IBUS
        ...
        // Creator cut and user yield
        const creatorCut = statsAfter.creatorUnclaimedYield - statsBefore.creatorUnclaimedYield;
        const userYield = await getPendingYield(algod, deployment, alice.addr);

        // Total output = creator cut + user yield (the contract splits totalOutput between them)
        const totalOutput = creatorCut + userYield;

        // Verify creator fee is exactly CREATOR_FEE% of total (swap + farm bonus)
        const expectedCreatorCut = Math.floor(totalOutput * CREATOR_FEE / 100);
        expect(creatorCut).toBe(expectedCreatorCut);

        // Verify farm bonus is included (creator cut > what it would be without farm)
        const swapOnlyOutput = totalOutput - farmBonus;
        ...
        console.log('  Total output (swap + farm):', totalOutput / 1_000_000, 'IBUS');
        console.log('  Farm bonus:', farmBonus / 1_000_000, 'IBUS');
        console.log('  Creator cut (5% of total):', creatorCut / 1_000_000, 'IBUS');
        console.log('  Creator cut if no farm:', creatorCutWithoutFarm / 1_000_000, 'IBUS');
        console.log('  User yield:', userYield / 1_000_000, 'IBUS');
      });
  ...
  describe('10-Phase Real-World Scenario with Farm', () => {
    ...
    it('Phase 3: Enable farm', async () => {
      ...
      console.log('Phase 3: Farm enabled with 1000 IBUS, ratio:', farmStats.emissionRatio);
    });
    ...
    it('Phase 10: Complete close out', async () => {
      ...
      expect(stats.totalDeposits).toBe(0);
      // With dynamic emission, farm doesn't fully deplete (geometric decay)
      // Remaining swap asset is the undistributed farm balance + dust
      const farmStats = await getFarmStats(algod, deployment);
      expect(stats.swapAssetBalance).toBeCloseTo(farmStats.farmBalance, -2);
    });
  });
  ...
  describe('Emission Ratio Constraints', () => {
    ...
    it('should enforce 10% floor when dynamic rate would be lower', async () => {
      ...
      // Tiny farm (1 IBUS) with small ratio on large deposits → rate would be far below 10%
      await fundAsset(algod, creator, creator.addr, deployment.ibusAssetId, 10_000_000);
      await performContributeFarm(algod, deployment, creator, 1_000_000); // 1 IBUS
    ...
    it('should return correct currentDynamicRate from getFarmStats', async () => {
      ...
      // After a swap, farm depletes → rate should decrease
      await performSwapYield(algod, deployment, creator, 100_000_000, 100);
      ...
      console.log('  Before swap: rate =', farmStats.currentDynamicRate, 'bps (' + (farmStats.currentDynamicRate / 100).toFixed(1) + '%)');
      console.log('  After swap: rate =', farmStatsAfter.currentDynamicRate, 'bps (' + (farmStatsAfter.currentDynamicRate / 100).toFixed(1) + '%), farm:', farmStatsAfter.farmBalance / 1_000_000, 'IBUS');
    });

    it('should return rate 0 when all users withdraw (totalDeposits = 0)', async () => {
      const deployment = await deployVaultForTest(algod, creator, {
        creatorFeeRate: 0,
        minSwapThreshold: 2_000_000,
      });
      ...
      console.log('Rate = 0 when totalDeposits = 0, farm still has', farmStats.farmBalance / 1_000_000, 'IBUS');
    });
  });
  ...
  describe('Min Swap Threshold Update', () => {
    ...
    it('should reject createVault with threshold above 50 USDC', async () => {
      await expect(
        deployVaultForTest(algod, creator, {
